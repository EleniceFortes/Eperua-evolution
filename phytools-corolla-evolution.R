...
floral.type<-setNames(eperua.trait[,2], rownames(eperua.trait))
floral.type

##set colors for plotting
cols<-setNames(c("black","grey","purple"),levels(floral.type))
cols
##plot tree & data
...
mtrees1<-make.simmap(eperua.tree,floral.type,model="SYM",nsim=1000,
                     Q="mcmc",vQ=0.01,prior=list(use.empirical=T),samplefreq=10)
mtrees1
...
#tiff device
tiff("plot_corolla_cladogram_ER-1-last3.tiff", width = 10, height = 10, units = 'in', res = 600)
#plotting
par(mfrow=c(1,1))
plot(pd,colors=cols,fsize=1.2,ftype="i",lwd=2,
     offset=0.4,ylim=c(-1,Ntip(eperua.tree)),cex=c(0.7,0.45))
legend("topleft",legend=levels(floral.type),pch=22,pt.cex=4.5,
